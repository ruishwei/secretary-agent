...
export function executeMemoryReplace(memoryStore: MemoryStore): ToolHandler {
  return {
    ...
    async execute(args) {
      const target = args.target as "deep" | "user";
      const oldStr = args.old as string;
      const newStr = args.new as string;

      if (!target || !oldStr || !newStr) {
        return { success: false, result: "", error: "target (deep|user), old, and new are required" };
      }

      if (target !== "deep" && target !== "user") {
        return { success: false, result: "", error: "target must be 'deep' or 'user'" };
      }
      ...
      const label = target === "deep" ? "deep memory (MEMORY.md)" : "USER.md";
